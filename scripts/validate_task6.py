...
def run():
    ...
    try:
        ...
        for art in [inf_out, judged_out, analysis_out]:
            ...
            # check both .csv.meta.json variant or appended .meta.json are acceptable
            alt = Path(art).with_suffix(Path(art).suffix + ".meta.json")
            assert meta.exists() or alt.exists(), f"provenance missing for {art}"
    finally:
